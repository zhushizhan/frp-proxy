<template>
  <el-form-item v-if="!readonly" :label="label" :prop="prop" :class="($attrs.class as string)">
    ...
    <PopoverMenu
      ...
      full-width
      filterable
      :filter-placeholder="placeholder || t('common.searchEllipsis')"
      @update:model-value="$emit('update:modelValue', $event)"
    >
...
const displayValue = computed(() => {
  if (props.modelValue == null || props.modelValue === '') return t('common.empty')
  if (props.type === 'select') {
    const opt = props.options.find((o) => o.value === props.modelValue)
    return opt ? opt.label : String(props.modelValue)
  }
  if (props.type === 'password') {
    return props.modelValue ? '******' : t('common.empty')
  }
