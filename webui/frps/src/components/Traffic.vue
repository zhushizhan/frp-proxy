<template>
  <div class="traffic-chart-container" v-loading="loading">
      ...
      <div class="bars-area">
        ...
        <div v-for="(item, index) in chartData" :key="index" class="day-column">
          <div class="bars-group">
            ...
            <el-tooltip
              :content="t('traffic.tooltipOut', { value: formatFileSize(item.out) })"
              placement="top"
            >
              <div
                class="bar bar-out"
    ...
    <div v-if="!loading && chartData.length > 0" class="legend">
      <div class="legend-item"><span class="dot in"></span> {{ t('traffic.legendIn') }}</div>
      <div class="legend-item"><span class="dot out"></span> {{ t('traffic.legendOut') }}</div>
    </div>
...
const fetchData = () => {
  ...
  getProxyTraffic(props.proxyName)
    ...
    .catch((err) => {
      ElMessage({
        showClose: true,
        message: t('traffic.fetchFailed', { message: String(err) }),
        type: 'warning',
      })
    })
    .finally(() => {
