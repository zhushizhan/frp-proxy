  ...
  <div class="clients-page">
    ...
    <div v-loading="loading" class="clients-content">
      ...
      <div v-else-if="!loading" class="empty-state">
        <el-empty :description="t('clients.empty')" />
      </div>
    </div>
  </div>
</template>
...
const stats = computed(() => {
  ...
  const online = clients.value.filter((client) => client.online).length
...
const filteredClients = computed(() => {
  ...
  if (statusFilter.value === 'online') {
    result = result.filter((client) => client.online)
  } else if (statusFilter.value === 'offline') {
    result = result.filter((client) => !client.online)
  }

  if (searchText.value) {
    result = result.filter((client) => client.matchesFilter(searchText.value))
  }
...
const fetchData = async () => {
  ...
  try {
    ...
    ElMessage({
      showClose: true,
      message: t('clients.fetchFailed', { message: error.message }),
      type: 'error',
    })
  } finally {
    loading.value = false
