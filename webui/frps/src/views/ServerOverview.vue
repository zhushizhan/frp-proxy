<template>
  <div class="server-overview">
    <el-row :gutter="20" class="stats-row">
      ...
      <el-col :xs="24" :sm="12" :lg="6">
        <StatCard
          :label="t('overview.connectionsLabel')"
          :value="data.curConns"
          type="connections"
    ...
    <el-card class="config-card" shadow="hover">
      ...
      <div class="config-grid">
        ...
        <div class="config-item">
          ...
          <span class="config-value">{{ displayMaxPortsPerClient }}</span>
        </div>
...
const hasActiveProxies = computed(() =>
  Object.values(data.value.proxyTypeCounts).some((count) => count > 0),
)

const displayMaxPortsPerClient = computed(() =>
  data.value.maxPortsPerClient === 0
    ? t('common.noLimit')
    : String(data.value.maxPortsPerClient),
)
...
const fetchData = async () => {
  try {
    ...
    data.value.maxPortsPerClient = Number(json.maxPortsPerClient || 0)
